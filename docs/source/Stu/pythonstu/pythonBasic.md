...
### 5.2. python中的标准数据类型
- 共6种
  - **String 字符串型**：有下标(index)，有顺序，可迭代，不可修改；
  - **Number 数值型**；
  - **List 列表型**：有下标，有顺序，可迭代，可修改。；
  - **Tuple 元组型**：有下标，有顺序，可迭代，不可修改；
  - **Dictionary 字典类型**：可迭代,可修改，key是唯一且不可变的(整型、字符串、元组)；
...
## 10. 模块与包
### 10.1. 含义
- **模块**：就是 `.py` 文件，有内置模块，第三方模块，自定义模块。
- **包**：本质上是一个目录，在这个目录中存在一个 `__init__.py` 文件，用于管理模块（即在包中可以存在若干的 `.py` 文件）。
...
- 通过`pip`命令安装：
  - `pip list`：列出当前已经安装的第三方库；
  - `pip show 模块名`：列出指定库的基本信息；
  - `pip install 模块名`：安装指定模块（最新版）；
  - `pip install 模块名==版本号`：安装指定模块（指定版本），版本号省略可列出所有能安装的版本；
  - `pip uninstall 模块名`：卸载；
  - `pip install 模块名 -i 镜像源地址`：从此镜像源下载该模块；
