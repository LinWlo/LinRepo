...
- **封装**
  - 私有化：在属性或者方法名前面加2条下划线，变为私有属性或者私有方法，只能在类的内部方位。可以在类中提供相应的接口供外部访问私有属性或者私有方法，从而控制外部访问的权限或者方式。
  - 例子
...
- **继承**
  ...
  - **多继承**：继承几个父类，如果多个父类有相同的属性名或者方法名，使用哪一个父类的同名属性或者同名方法？ 即会自动查找，遇到第一个就停止。
  - 基本概念
    - 父类：被其他类继承的类，也叫基类，超类。
    - 子类：继承其他类的类，一个子类可以有若干父类，可以继承父类中所有的非私有的属性和方法。
    - 方法重写：子类继承父类的方法后，重新实现该方法。
    - 即使用父类的同名方法作为方法名，重写代码。
  - 注：
    - 当继承多个类时，不同的类同名方法时，访问继承时写在前面的类的方法。
    - 如果父类和子类有同名的属性或方法，子类以自己为准（类似于方法重写）。
    - 子类访问属性或方法时，先找自己，如果没有就去第一个父类找，父类也没有的话，就继续往上一个父类找（最后找到object超级基类，还是没有的话就会去第二个父类找………，最后找完都找不到就会报错。
  - 继承中的构造方法
    - 父类有构造方法，子类中没有重写构造方法：子类默认继承第一个父类的构造方法。
    - 父类，子类都有构造方法：子类实例化时访问的是子类的构造方法。
    - 父类，子类都有构造方法，并且子类需要访问父类的构造方法。
    - 注：只能在子类的构造方法中访问父类的构造方法。
    - 实现方法：
      - super(子类名,self)._ _init_ _()：访问第一个父类的构造方法。
      - 父类名._ _init_ _(self)：访问指定父类的构造方法。
- 多态
  - 相同的方法（名），通过不同的对象去调用该方法可以呈现不同的结果。
  - 像pycharm面向对象例子里面的饲养员和动物，狼和羊都有cry和eat的方法，但通过不同对象调用有不用效果，而且这样写后面要增加狗，猪等其他动物的话可以直接加上类就行，对饲养员类没有影响。
